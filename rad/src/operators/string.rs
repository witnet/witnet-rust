...
pub fn string_match(input: &RadonString, args: &[Value]) -> Result<RadonTypes, RadError> {
    ...
    let unsupported_operator = RadError::UnsupportedOperator {
        input_type: "RadonString".to_string(),
        operator: "String match".to_string(),
...
mod tests {
    ...
    fn test_string_match_booleans() {
        ...
        map.insert(Value::Text("value1".to_string()), Value::Bool(true));
        map.insert(Value::Text("value2".to_string()), Value::Bool(false));

        let mut input_key = RadonString::from("value1");
        ...
        input_key = RadonString::from("value2");

        let result = string_match(&input_key, &args);
        assert_eq!(result.unwrap(), RadonTypes::from(RadonBoolean::from(false)));

        input_key = RadonString::from("value3");
    ...
    fn test_string_match_integers() {
        let mut map: BTreeMap<Value, Value> = BTreeMap::new();
        map.insert(Value::Text("value1".to_string()), Value::Integer(1));
        map.insert(Value::Text("value2".to_string()), Value::Integer(2));
    ...
    fn test_string_match_floats() {
        let mut map: BTreeMap<Value, Value> = BTreeMap::new();
        map.insert(Value::Text("value1".to_string()), Value::Float(1.0));
        map.insert(Value::Text("value2".to_string()), Value::Float(2.0));
    ...
    fn test_string_match_wrong_arguments() {
        ...
        let input_key = RadonString::from("value1");
    ...
    fn test_string_match_mismatched_types() {
        let mut map: BTreeMap<Value, Value> = BTreeMap::new();
        map.insert(Value::Text("value1".to_string()), Value::Float(1.0));
        map.insert(Value::Text("value2".to_string()), Value::Bool(true));
    ...
    #[test]
    fn test_string_match_default_no_native() {
        let mut map: BTreeMap<Value, Value> = BTreeMap::new();
        map.insert(Value::Text("value1".to_string()), Value::Float(1.0));
        map.insert(Value::Text("value2".to_string()), Value::Float(2.0));
        ...
        let args = vec![Value::Map(map), Value::Array([].to_vec())];

        let result = string_match(&input_key, &args);
        println!("{:?}", result);
        assert_eq!(result.unwrap_err().to_string(), "Call to operator `RadonString` with args `Some([Map({Text(\"value1\"): Float(1.0), Text(\"value2\"): Float(2.0)}), Array([])])` is not supported for input type `String match`");
    }
    ...
    fn test_string_match_default_insufficient_arguments() {
        ...
        let result = string_match(&input_key, &args);
        assert_eq!(result.unwrap_err().to_string(), "Wrong `RadonString::String match()` arguments: `[Map({Text(\"value1\"): Float(1.0), Text(\"value2\"): Float(2.0)})]`");
    }
