...
impl RadonType<LocalResult> for RadonResult {
    ...
    fn radon_type_name() -> String {
        String::from(RADON_RESULT_TYPE_NAME)
    }
}
...
impl TryFrom<RadonTypes> for RadonResult {
    type Error = RadError;

    fn try_from(item: RadonTypes) -> Result<Self, Self::Error> {
        if let RadonTypes::Result(rad_result) = item {
            Ok(rad_result)
        } else {
    ...
    fn operate(&self, call: &RadonCall) -> Result<RadonTypes, RadError> {
        match call {
            (op_code, args) => Err(RadError::UnsupportedOperator {
                input_type: String::from(RADON_RESULT_TYPE_NAME),
