...
impl Operable for RadonMap {
    ...
    fn operate_in_context(
        ...
        match call {
            ...
            (RadonOpCodes::MapAlter, Some(args)) => {
                map_operators::alter(self, args, context).map(RadonTypes::from)
            }
            (RadonOpCodes::MapPick, Some(args)) => {
                map_operators::pick(self, args).map(RadonTypes::from)
            }
            (RadonOpCodes::MapStringify, None) => {
                map_operators::stringify(self).map(RadonTypes::from)
            }
