    ...
    T: Database + GetWith,
{
    ...
    pub fn index_block_transactions(
        ...
        if confirmed {
            ...
            let block_state = state::StateSnapshot {
                ...
                stake_output_set: state.stake_output_set.clone(),
            };
    ...
    fn _persist_block_txns(
        ...
        stake_output_set: model::StakeOutputSet,
    ...
    fn _get_account_mutation(
        ...
        let miner_fee: u64 = match &txn.metadata {
            Some(model::TransactionMetadata::InputValues(input_values)) => {
                ...
                match txn.transaction.clone() {
                    Transaction::Stake(st) => total_input_amount
                        .saturating_sub(st.body.output.value)
                        .saturating_sub(st.body.change.map_or(0, |change| change.value)),
                    // Genesis block (no inputs) or empty block with only `MintTransaction`
                    _ if total_input_amount == 0 => 0,
                    _ => {
                        ...
                        total_input_amount
                            ...
                            .unwrap_or_else(|| {
                                log::warn!(
                "Miner fee below 0 in a transaction of type value transfer or data request: {}",
                txn.transaction.versioned_hash(protocol_version)
            );
                                0
                            })
                    }
        ...
        Ok(Some(AccountMutation {
            ...
            utxo_inserts,
            utxo_removals,
            stake_output_inserts,
            stake_output_removals,
        }))
    }
    ...
    pub fn try_consolidate_block(&self, block_hash: &str) -> Result<()> {
        ...
        self._persist_block_txns(
            ...
            block_state.stake_output_set.clone(),
            &block_state.balance,
            &block_state.beacon,
        )?;
