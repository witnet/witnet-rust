...
impl TransactionsPool {
    ...
    /// Remove an unstake transaction from the pool and make sure that other transactions
    /// that may try to spend the same UTXOs are also removed.
    /// This should be used to remove transactions that got included in a consolidated block.
    ///
    /// Returns an `Option` with the value transfer transaction for the specified hash or `None` if not exist.
    ...
    /// # use witnet_data_structures::transaction::{Transaction, UnstakeTransaction};
    /// let mut pool = TransactionsPool::new();
    /// let vt_transaction = UnstakeTransaction::default();
    /// let transaction = Transaction::Unstake(ut_transaction.clone());
    /// pool.insert(transaction.clone(),0);
    ///
    /// assert!(pool.ut_contains(&transaction.hash()));
    ///
    /// let op_transaction_removed = pool.ut_remove(&ut_transaction);
    ///
    /// assert_eq!(Some(ut_transaction), op_transaction_removed);
    /// assert!(!pool.ut_contains(&transaction.hash()));
    /// ```
    pub fn ut_remove(&mut self, tx: &UnstakeTransaction) -> Option<UnstakeTransaction> {
        let key = tx.hash();
        let transaction = self.ut_remove_inner(&key, true);
    ...
    /// block.
    /// If the transaction did get included in a consolidated block, use `st_remove` instead.
    fn ut_remove_inner(&mut self, key: &Hash, consolidated: bool) -> Option<UnstakeTransaction> {
        // TODO: is this taking into account the change and the stake output?
        self.ut_transactions
            ...
            .map(|(weight, transaction)| {
                ...
                // TODO?
                // if !consolidated {
                //     self.remove_tx_from_output_pointer_map(key, &transaction.body.);
                // }
                transaction
            })
    }
