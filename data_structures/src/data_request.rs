...
impl DataRequestPool {
    ...
    pub fn get_reveals(&self, dr_pointer: &Hash) -> Option<Vec<RevealTransaction>> {
        self.data_request_pool.get(dr_pointer).map(|dr_state| {
            let mut pkhs: Vec<PublicKeyHash> = dr_state.info.reveals.keys().cloned().collect();

            pkhs.sort();
            let reveals: Vec<RevealTransaction> = pkhs
                .iter()
                .map(|pkh| dr_state.info.reveals.get(pkh).unwrap().clone())
                .collect();
        ...
        self.data_request_pool
            ...
            .filter_map(|(dr_pointer, dr_state)| {
                if let DataRequestStage::TALLY = dr_state.stage {
                    let mut pkhs: Vec<PublicKeyHash> =
                        dr_state.info.reveals.keys().cloned().collect();
                    pkhs.sort();
                    let reveals: Vec<RevealTransaction> = pkhs
                        .iter()
                        .map(|pkh| dr_state.info.reveals.get(pkh).unwrap().clone())
...
where
    RT: TypeLike,
{
    if let Stage::Tally(tally_metadata) = report.metadata.clone() {
