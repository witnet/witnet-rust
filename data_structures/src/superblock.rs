...
mod tests {
    ...
    fn superblock_state_initial_non_empty() {
        ...
        let sb1 = sbs.build_superblock(&block_headers, &ars2, &[], 0, Hash::default());
    ...
    fn superblock_state_first_superblock_cannot_be_validated() {
        ...
        let bls_pk =
            Bn256PublicKey::from_secret_key(&Bn256SecretKey::from_slice(&[1; 32]).unwrap())
                .unwrap();
        let sb1 = sbs.build_superblock(&block_headers, &ars_identities, &[bls_pk], 0, genesis_hash);
    ...
    fn superblock_state_already_seen() {
        ...
        let _sb2 =
            sbs.build_superblock(&block_headers, &ars_identities, &[bls_pk], 1, genesis_hash);
    ...
    fn superblock_state_double_vote() {
        ...
        let _sb0 = sbs.build_superblock(&block_headers, &ars0, &ars0_ordered, 0, genesis_hash);
        ...
        let _sb1 = sbs.build_superblock(&block_headers, &ars1, &ars1_ordered, 1, genesis_hash);

        // The ARS included in superblock 1 contains only identity p1, so only its vote will be
        // valid in superblock votes for index 2
        let sb2 = sbs.build_superblock(&block_headers, &ars2, &ars2_ordered, 2, genesis_hash);
    ...
    fn superblock_state_ars_identities() {
        ...
        // Superblock votes for index 0 cannot be validated because we do not know the ARS for index -1
        // (because it does not exist)
        let sb0 = sbs.build_superblock(&block_headers, &ars0, &ars0_ordered, 0, genesis_hash);
        ...
        // The ARS included in superblock 0 is empty, so none of the superblock votes for index 1
        // can be valid, they all return `NotInArs`
        let sb1 = sbs.build_superblock(&block_headers, &ars1, &ars1_ordered, 1, genesis_hash);
        ...
        let sb3 = sbs.build_superblock(&block_headers, &ars3, &ars3_ordered, 3, genesis_hash);
        ...
        let sb4 = sbs.build_superblock(&block_headers, &ars4, &ars4_ordered, 4, genesis_hash);
