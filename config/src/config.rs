...
pub struct Log {
    ...
    #[partial_struct(serde(deserialize_with = "as_log_filter", serialize_with = "as_log_filter_string"))]
    pub level: log::LevelFilter,
    /// Automated bug reporting (helps the community improve the software)
    pub sentry_telemetry: bool,
}

fn as_log_filter_string<S>(level: &Option<log::LevelFilter>, serializer: S) -> Result<S::Ok, S::Error> where S:serde::Serializer {
    if let Some(level_unwrapped) = level {
        ...
        return serializer.serialize_str("info");
    }
}
...
fn to_millis<S>(val: &Option<Duration>, serializer: S) -> Result<S::Ok, S::Error> where S: serde::Serializer {
...
fn to_secs<S>(val: &Option<Duration>, serializer: S) -> Result<S::Ok, S::Error> where S: serde::Serializer {
    if let Some(duration) = val {
...
fn to_protected_string<S>(_val: &Option<Protected>, serializer: S) -> Result<S::Ok, S::Error> where S: serde::Serializer {
...
impl Connections {
    ...
    pub fn to_partial(&self) -> PartialConnections {
        PartialConnections {
            ...
            bootstrap_peers_period: Some(self.bootstrap_peers_period.clone()),
            storage_peers_period: Some(self.storage_peers_period.clone()),
            discovery_peers_period: Some(self.discovery_peers_period.clone()),
            check_melted_peers_period: Some(self.check_melted_peers_period.clone()),
            feeler_peers_period: Some(self.feeler_peers_period.clone()),
            handshake_timeout: Some(self.handshake_timeout.clone()),
            handshake_max_ts_diff: Some(self.handshake_max_ts_diff.clone()),
