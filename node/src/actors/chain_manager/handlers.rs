                                ...
                                self.construct_superblock(ctx, epoch_during_which_we_should_construct_the_target_superblock)
                                    .and_then(move |superblock, act, ctx| {
                                        match superblock {
                                            ...
                                            None => {
                                                // This can happen if there is a race condition
                                                // when persisting blocks and superblock creation
                                                // starts before the blocks are stored.
                                                // Or it can also happen if our peers lied to us.
                                                // In that case we should drop all peers and go to
                                                // waiting consensus, but the first option looks
                                                // more possible so panic.
                                                panic!("There were no blocks to create the superblock, but this superblock should exist according to our outbound peers");
                                            }
                                        }
